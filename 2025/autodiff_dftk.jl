...
Applications include uncertainty propagation, property derivatives,
numerical error estimation, and machine learning DFT functionals. 
...
system = pyconvert(AbstractSystem, ase.build.bulk("MgO", "rocksalt", a=4.26, cubic=true))

# ╔═╡ bc258972-17a3-4112-b56b-751e044694f9
md"## Model parameters in DFT"
...
begin
	...
	# Numerical parameters (very crude)
	kgrid = (2, 2, 2)
	Ecut = 20
	
	model0 = model_DFT(system; pseudopotentials, functionals)
	basis0 = PlaneWaveBasis(model0; Ecut, kgrid)
	scfres = self_consistent_field(basis0; tol=1e-6)
end
...
md"### Setup part 1: Exposing Pseudopotential parameters"

# ╔═╡ 562f4688-8e29-4c36-9308-976a27d25785
function parameters_psp(psp::PspHgh)
    ComponentVector(; psp.rloc, psp.cloc, rp0=psp.rp[1], rp1=psp.rp[2], h0=psp.h[1], h1=psp.h[2])
end

# ╔═╡ bda2daaa-fdfa-4d6c-a766-54b4039ec61a
function make_psp(psp::PspHgh, θ::AbstractVector{T}) where {T}
    PspHgh(psp.Zion, θ.rloc, θ.cloc, [θ.rp0, θ.rp1], [θ.h0, θ.h1])
end

# ╔═╡ bd4f9f06-77bc-4eda-bf3b-03ca487827b3
md"Let's say we are interested in the GTH pseudopotential parameters of $\textrm{Mg}$ in the given system above."

# ╔═╡ 2d2db7cc-a5fc-4d08-80d2-b7d4aba0f79f
X0 = ElementPsp(:Mg, load_psp(pseudopotentials[:Mg]))

# ╔═╡ eb6748ea-9083-479e-8636-fb2c44b14ae4
θ_psp = parameters_psp(X0.psp)

# ╔═╡ 11d5cca7-3d82-4fd5-b1eb-c4ef9fc9882c
md"### Setup part 2: Exposing XC parameters"

# ╔═╡ cbbadd5f-215b-4de5-89d3-b0a7e5b01b80
make_pbe(θ) = [PbeExchange(θ), DftFunctional(:gga_c_pbe)]

# ╔═╡ 38d7dcf2-71cf-4276-8c91-5d3c9595e89f
θ_xc = parameters(DftFunctional(:gga_x_pbe))

# ╔═╡ 8ec1934c-c6d9-407d-bf0d-3f5adbcf5bd9
md"""
## The "forward pass" of DFT: From parameters to ground-state density
"""

# ╔═╡ 719b0fcb-d926-489d-b573-3dd5db29ceb5
function compute_ground_state(θ::AbstractVector{T}; Ecut=20, kgrid=2, tol=1e-6) where {T}
    lattice = T.(model0.lattice)  # Need to type-cast the model for ForwardDiff

    # Construct and insert the pseudopotential
    psp = make_psp(X0.psp, θ)
    X = ElementPsp(element_symbol(X0), psp)
    atoms = map(model0.atoms) do atom
        (element_symbol(atom) == element_symbol(X)) ? X : atom
    end

    # Construct and insert the XC functional
    xc = Xc(make_pbe(θ))
    terms = map(model0.term_types) do term
        term isa Xc ? xc : term
    end

    model = Model(model0; lattice, atoms, terms)

    # Discretize the PDE
    basis = PlaneWaveBasis(model; Ecut, kgrid=(kgrid, kgrid, kgrid))
...
# ╔═╡ ed06f5d4-48ce-4cd8-961f-f7e707ea9369
# Concatenate XC and PSP parameters for this example
θ = ComponentVector(; θ_xc..., θ_psp...)

# ╔═╡ 8b2eaa54-857e-48fa-b7de-a71d219db99b
# A simple SCF
ρ = compute_ground_state(θ);
...
For simplicity, in this example we just focus on the response of the real-space representation of the density. 
"""

# ╔═╡ 9d62563c-7674-4da3-8d1b-9c5cee437f56
jac = @time ForwardDiff.jacobian(
    x -> compute_ground_state(ComponentVector(; θ..., κ=x[1], rloc=x[2])),
    [θ.κ, θ.rloc]
);

# ╔═╡ 867aca1c-6374-456d-9232-46c532525b50
function plot_density_derivatives(scfres, jac)
    fft_size = scfres.basis.fft_size

    fig = Figure(size=(900,400))
    z_idx = Int(round(fft_size[3] / 2))

    ax = Makie.Axis(fig[1,1], aspect=AxisAspect(1), title="ρ")
    ax.tellheight = true
    hidedecorations!(ax)
    heatmap!(ax, scfres.ρ[:,:,z_idx,1])

    colormap = :diverging_bwr_55_98_c37_n256
    
    ax = Makie.Axis(fig[1,2], aspect=AxisAspect(1), title="XC: ∂ρ / ∂κ")
    hidedecorations!(ax)
    δρ = reshape(jac[:,1], fft_size...)
    heatmap!(ax, δρ[:,:,z_idx]; colormap, colorrange=(-maximum(abs, δρ), maximum(abs, δρ)))

    ax = Makie.Axis(fig[1,3], aspect=AxisAspect(1), title="Pseudopotential: ∂ρ / ∂rloc")
    hidedecorations!(ax)
    δρ = reshape(jac[:,2], fft_size...)
    colorrange = (-maximum(abs, δρ), maximum(abs, δρ))
    heatmap!(ax, δρ[:,:,z_idx]; colormap, colorrange=(-maximum(abs, δρ), maximum(abs, δρ)))
...
# ╔═╡ afc2925f-f7ef-4392-b605-1a07a6cddfd7
display(system) # See coordinates of atoms to aid visual interpretation

# ╔═╡ ff51f2c6-56bd-4a30-8ec1-eb0db8d139f9
plot_density_derivatives(scfres, jac)
...
# ╔═╡ 88b8d754-37de-42f9-be45-4f7c5c2a8915
md"## Research interests"

# ╔═╡ dce1cc1b-0998-48e0-92d5-b71bee256893
md"""
This opens up novel algorithmic approaches for exploration:
- Gradient-based **inverse design** of geometric structures to target properties
- **Machine learning** of model parameters on bulk observables
    - Finetuning XC parameters towards experimental data
    - Finetuning pseudopotentials towards all-electron data such as energy-volume curves
- Forward propagation of **model uncertainty** based on derivatives
- Forward propagation of perturbative **discretization error estimates**

For more detailed examples, see our forthcoming article on ForwardDiff in DFTK[1]
...
git-tree-sha1 = "7c0886a87b7b626f2fdf0697c30ad18deac469ca"
...
[[deps.AtomsView]]
deps = ["AtomsBase", "Bio3DView", "LinearAlgebra", "Unitful", "UnitfulAtomic"]
git-tree-sha1 = "63b8bad804e815c053c5f190c8a1bff7a6cf3c99"
uuid = "ee286e10-dd2d-4ff2-afcb-0a3cd50c8041"
version = "0.1.4"
...
[[deps.Bio3DView]]
git-tree-sha1 = "fcbfbe4dcab4a81aa21b2643fe3a29e795760b3e"
uuid = "99c8bb3a-9d13-5280-9740-b4880ed9c598"
version = "1.0.0"

    [deps.Bio3DView.extensions]
    Bio3DViewBioStructuresExt = "BioStructures"
    Bio3DViewBlinkExt = "Blink"

    [deps.Bio3DView.weakdeps]
...
git-tree-sha1 = "767ed4395a27df05bcd9731e02c6558fddb202c5"
...
git-tree-sha1 = "e18c95f6a68e279c738717da52ab55ea2bf4c688"
