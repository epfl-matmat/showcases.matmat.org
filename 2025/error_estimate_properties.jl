...
$e_{\mathbf{G}}(\mathbf{x}) = \frac{e^{i \mathbf{G} \cdot \mathbf{x}}}{\sqrt{\Gamma}}  \qquad \mathbf{G} \in \mathcal{R}^*$

The vectors in the basis are chosen according to a kinetic energy cutoff $E_{\mathrm{cut}}$, such that only the $\mathbf{G}$ vectors with a lower kinetic energy are included in the basis, i.e. such that

$\frac{1}{2} G^2 \le E_{\mathrm{cut}}.$

The basis size scales as $\mathcal O\left(\left(E_{\mathrm{cut}}\right)^{\frac32}\right)$, and therefore so does the computational time. $E_{\mathrm{cut}}$ is a numerical parameter that needs to be chosen to balance accurancy and computational time.

Our research in this area builds on top of [^CDKL2022], which proposes a practical way to estimate the error stemming from the choice of a finite $E_{\mathrm{cut}}$, such that it can be **computed** at a moderate cost, and partially **corrected for**. In particular, the error on the computed atomic forces is estimated. The atomic forces are in important quantity of interest. Notably, they are used in the context of geometry optimization (to find the equilibrium atom positions), ab initio molecular dynamics (to move the atoms over time) and as a reference to train interatomic force fields.
...
md"""
## Practical example
This section walks through an example, computed on a silicon crystal. All computations are performed with the [DFTK.jl](https://docs.dftk.org) package, which contains an implementation of the method from [^CDKL2022], [with a corresponding tutorial](https://docs.dftk.org/stable/examples/error_estimates_forces/).

We first construct a DFT model for silicon, using the PBE functional:
"""
...
model = let
	...
	atoms     = [Si, Si]
	positions = [ones(3)/8, -ones(3)/8 + [0.24, -0.33, 0.12]/20]
	# Build the DFT model
...
md"""
We will use a fixed $4 \times 4 \times 4$ $k$-grid for all simulations in this notebook:
"""

# ╔═╡ e1b734b9-0476-4f8a-96b9-58b15aa7b9fc
kgrid = [4, 4, 4] # (Regular Monkhorst-Pack grid)

# ╔═╡ c6fe96bb-f410-4239-89ca-ae73543fe4df
md"""
### Reference computation
We first perform a reference computation, with $E_{\mathrm{cut}} = 24 \, \mathrm{Ha}$. For this system, this corresponds to a high value of $E_{\mathrm{cut}}$.
"""

# ╔═╡ d2be9607-2d47-4edd-b538-607fa5c7e99c
md"""
The first step is to construct the corresponding plane-wave basis:
"""

# ╔═╡ ae30a0f3-91a5-49de-85be-1a5a4c29374a
reference_basis = PlaneWaveBasis(model; Ecut=24, kgrid);

# ╔═╡ cd36aecd-c549-45fb-8d1c-cd30040ac506
md"""
We can then perform a self-consistent field (SCF) calculation to solve the DFT problem:
"""

# ╔═╡ b0dafe89-7e9f-4014-bb56-be7908078b00
reference_scfres = self_consistent_field(reference_basis; tol=1e-8);

# ╔═╡ 6e2914f3-df7d-4901-83f6-2dd5af4a212c
md"""
And compute the corresponding atomic forces:
"""

# ╔═╡ 1933c528-e411-4be2-9ce6-1cf368cc3035
reference_forces = compute_forces_cart(reference_scfres)

# ╔═╡ d8d6cb35-4563-4fae-986c-e6c893db7950
md"""
### Error bound computations
We now perform computations with much lower choices of $E_{\mathrm{cut}} = 7,8,\ldots,17 \, \mathrm{Ha}$.

Let's define a helper function to:
- Construct a basis for the lower $E_\mathrm{cut}$.
- Perform the SCF on the lower basis.
- Compute the error estimate relative to the reference basis (the one that uses $E_{\mathrm{cut}} = 24 \, \mathrm{Ha}$), using the technique from [^CDKL2022].
- Compute the atomic forces $F$ and their estimated error $\Delta F$, and convert the forces from unit cell coordinates to cartesian coordinates.

The error estimates are first computed using the `refine_scfres` function, which computes corrections for the electron densities and wavefunctions.
Then, `refine_forces` propagates this error through the computation of forces, using the automatic differentation capabilities of DFTK. (TODO: cross-reference AD notebook)
"""

# ╔═╡ d8bb87cc-97dc-4be7-a3e9-3ecbc5c27f5d
function compute_and_estimate_error(Ecut)
	# Construct a basis for the lower Ecut
	basis  = PlaneWaveBasis(model; Ecut, kgrid)
	# Perform the SCF for the lower Ecut
	@info "Performing SCF for Ecut=$Ecut"
	scfres = self_consistent_field(basis; tol=1e-8)

	# Compute corrections to the electron densities and wavefunctions in the reference basis
	@info "Computing corrections for Ecut=$Ecut"
	refinement = refine_scfres(scfres, reference_basis; tol=1e-6)
	# Propagate these corrections to the force
	(; F, dF) = refine_forces(refinement)
	# Convert the force F and its correction dF to cartesian coordinates
	base_forces = DFTK.covector_red_to_cart.(model, F)
	force_corrections = DFTK.covector_red_to_cart.(model, dF)

	(; Ecut, base_forces, force_corrections)
end

# ╔═╡ 1e276c3a-af00-44a2-a4f1-d659c650f4b1
md"""
And then call this function for $E_{\mathrm{cut}} = 7,8,\ldots,17 \, \mathrm{Ha}$:
"""

# ╔═╡ f3f833ba-3ef2-476f-82ba-2b433e2a7525
computations = map(7:17) do Ecut
	Ecut => compute_and_estimate_error(Ecut)
end

# ╔═╡ 8335dc5d-c0e7-47ae-9d4c-f0030707c3ae
md"""
### Visual comparison
Let's call $F_\ast$ the reference atomic forces. To assess whether the error estimates are good, we can compare the estimated error $\Delta F$ against the real error $F_\ast - F$.

A first observation in the following plot is that forces tend to converge to the reference as $E_\mathrm{cut}$ is increased. For all considered values of $E_\mathrm{cut}$, excellent agreement is observed between the exact error (which would not be computed in practice) and the estimated error (which is computable in practice).
"""

# ╔═╡ a9de2f22-a480-4d79-9e2d-0756499d2252
let
	function force_norm(F)
		maximum(norm, F) * ustrip(u"eV/Å", 1u"hartree/bohr")
	end

	gr(size=(600, 600))
	p = plot(; xscale=:log10, yscale=:log10, annotationfontsize=10, annotationhalign=:left, xlabel="Exact discretization error in the force [eV/Å]", ylabel="Estimated discretization error in the force [eV/Å]")

	Ecuts = [string(Ecut) for (Ecut, _) in computations]
	# Collect F_∗ - F for all Ecuts:
	xs = [force_norm(reference_forces - c.base_forces) for (_, c) in computations]
	# Collect ΔF for all Ecuts:
	ys = [force_norm(c.force_corrections) for (_, c) in computations]

	# Scatter and annotate with the Ecut label
	scatter!(xs, ys; label="refinement")
	annotate!(collect(zip(xs * 1.1, ys, Ecuts)))

	# Plot reference line
	range = [5e-5, 2e-2]
	plot!(range, range; ls=:dash, lw=2, label="perfect agreement")

	p
end

# ╔═╡ dff8f7c6-6db0-4be7-a09c-e067add7d1b3
md"""
## Research interests
As demonstrated on this simple silicon example, the error estimates from [^CDKL2022] are very promising. Our current research in this area is aimed at the following goals:
- Showing under which conditions these error bounds are accurate, for a large range of systems.
- Extending the formalism such that these error bounds can also be applied to metallic systems, which are currently unsupported.
- Extending the error bounds to other quantities of interest than atomic forces.
- Making use of the error estimates to lower the computational cost of DFT calculations while remaining within a chosen error threshold.
"""
...
manifest_format = "2.0"
project_hash = "353c841f2f6a8b6d34ff4b0354e4859e0be81fc8"
...
git-tree-sha1 = "c11624cf1e1766becf56223110f74747421c4d15"
...
git-tree-sha1 = "3d0127615715cf90203fc344713594877eb6365e"
...
[[deps.DFTK]]
deps = ["AbstractFFTs", "Artifacts", "AtomsBase", "AtomsCalculators", "Brillouin", "Dates", "DftFunctionals", "DifferentiationInterface", "DocStringExtensions", "FFTW", "ForwardDiff", "GPUArraysCore", "Interpolations", "IterTools", "KrylovKit", "Libxc", "LineSearches", "LinearAlgebra", "LinearMaps", "MPI", "Markdown", "Optim", "PeriodicTable", "PkgVersion", "Polynomials", "PrecompileTools", "Preferences", "Printf", "PseudoPotentialIO", "Random", "Roots", "SparseArrays", "SpecialFunctions", "Spglib", "StaticArrays", "Statistics", "TimerOutputs", "Unitful", "UnitfulAtomic"]
git-tree-sha1 = "9fcd218e28169ce134eb75f0872fd5eacb4ac752"
...
git-tree-sha1 = "e59c70bcecb8c00c5a30e5b27b3b3a00e704751f"
uuid = "25af9330-9b41-55d4-a324-1a83c0a0a1ac"
version = "7.0.0+0"

[[deps.Libxc_jll]]
deps = ["Artifacts", "CompilerSupportLibraries_jll", "JLLWrappers", "Libdl"]
git-tree-sha1 = "49065d5c50f82e9c1d98a8ede43cb595b6cf199e"
...
git-tree-sha1 = "922e0cd5bcaeb4033e7283e85eaa17e11e80c6d6"
